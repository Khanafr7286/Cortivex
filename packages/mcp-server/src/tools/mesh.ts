...
export async function meshTool(): Promise<{ content: Array<{ type: string; text: string }> }> {
  ...
  if (state.conflicts.length > 0) {
    const conflictLines = state.conflicts.map(
      (c) => `  - ${c.file}: claimed by ${c.claimedBy}, also requested by ${c.requestedBy} (${c.timestamp})`,
    );
