...
export async function stopTool(input) {
    ...
    await writeFile(join(signalDir, `stop-${input.runId}.json`), JSON.stringify({ runId: input.runId, signal: 'stop', timestamp: new Date().toISOString() }), 'utf-8');
    return {
        content: [{
                type: 'text',
                text: [
