...
import type { NodeRunState, PipelineRun } from '@cortivex/core';
...
export async function runCommand(
  ...
  console.log('');
  console.log(
    chalk.bold.cyan(`  Running pipeline: ${pipelineName}`)
  );
  ...
  if (options.learn !== false) {
    try {
      ...
      if (insights.length > 0) {
        ...
        const { pipeline: optimized, changes } = applier.apply(
          pipeline,
          insights
        );
        ...
        if (appliedChanges.length > 0) {
          ...
          console.log(
            chalk.magenta(
              `  Applied ${appliedChanges.length} learned optimization(s):`
            )
          );
          for (const change of appliedChanges) {
            console.log(
              chalk.magenta(`    - ${change.description}`)
            );
          }
          pipeline = optimized;
        }
  ...
  console.log(
    chalk.gray(`  Est. cost: ${pipeline.estimated_cost}`)
  );
  console.log(
    chalk.gray(`  Est. duration: ${pipeline.estimated_duration}`)
  );
  console.log('');
  ...
  executor.on('mesh:conflict', (file, claimedBy) => {
    console.log(
      chalk.yellow(
        `    Mesh conflict: "${file}" claimed by ${claimedBy}, waiting...`
      )
    );
  });
  ...
  const run = await executor.execute(pipeline, {
    ...
    failureStrategy: (options.strategy as 'stop' | 'continue' | 'retry') ?? 'stop',
...
function printProgress(
  ...
  process.stdout.write(
    `\r      ${bar} ${progress}% ${chalk.gray(message.slice(0, 40))}`
  );
  if (progress >= 100) {
    process.stdout.write('\n');
  }
...
function printNodeComplete(
  nodeId: string,
  state: NodeRunState
): void {
  const costStr =
    state.cost > 0 ? chalk.gray(` $${state.cost.toFixed(3)}`) : '';
  const tokensStr =
    state.tokens > 0 ? chalk.gray(` ${state.tokens} tokens`) : '';
  const filesStr =
  ...
  console.log(
    `  ${chalk.green('\u2714')} ${chalk.bold(nodeId)} ${chalk.green('completed')}${costStr}${tokensStr}${filesStr}`
  );
}

function printNodeFailed(nodeId: string, error: string): void {
  console.log(
    `  ${chalk.red('\u2718')} ${chalk.bold(nodeId)} ${chalk.red('failed')}`
  );
  console.log(chalk.red(`      ${error.slice(0, 200)}`));
}

function printSummary(run: PipelineRun, elapsed: string): void {
  console.log('');
  console.log(chalk.bold('  Pipeline Summary'));
  console.log(chalk.gray('  ' + '\u2500'.repeat(50)));

  const statusColor =
    run.status === 'completed' ? chalk.green : chalk.red;
  console.log(
    `  Status:     ${statusColor(run.status.toUpperCase())}`
  );
  console.log(
    `  Duration:   ${chalk.white(elapsed + 's')}`
  );
  console.log(
    `  Total Cost: ${chalk.white('$' + run.totalCost.toFixed(3))}`
  );
  console.log(
    `  Tokens:     ${chalk.white(run.totalTokens.toLocaleString())}`
  );

  // Node breakdown
  const completed = run.nodes.filter(
  ...
  console.log(
    `  Nodes:      ${chalk.green(completed + ' completed')}${failed > 0 ? chalk.red(', ' + failed + ' failed') : ''}${skipped > 0 ? chalk.yellow(', ' + skipped + ' skipped') : ''}`
  );

  if (run.filesModified.length > 0) {
    console.log(
      `  Modified:   ${chalk.white(run.filesModified.length + ' files')}`
    );
    ...
    if (run.filesModified.length > 10) {
      console.log(
        chalk.gray(
          `              ... and ${run.filesModified.length - 10} more`
        )
      );
    }
  }
