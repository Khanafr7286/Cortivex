...
class LearningEngineSingleton {
    ...
    const rec: ExecutionRecord = {
      ...
      nodeResults: run.nodes.map((n) => ({
        ...
        duration: n.startedAt && n.completedAt
          ...
          : 0,
        model: (n as unknown as Record<string, string>).model ?? 'claude-sonnet-4-20250514',
        error: n.error,
      })),
      repoContext: {
        languages: [],
  ...
  getInsights(pipeline?: string): Insight[] {
    if (!pipeline) return [...this.insights];
    return this.insights.filter(
      (i) => i.details.pipeline === pipeline || !i.details.pipeline,
    );
  }
  ...
  getStats(pipeline?: string): AggregateStats {
    ...
    const mostUsedPipeline = [...pipelineCounts.entries()].sort(
      (a, b) => b[1] - a[1],
    )[0]?.[0] ?? 'n/a';

    // Most expensive node type
    const nodeCosts = new Map<string, number>();
    const nodeFailures = new Map<string, { fail: number; total: number }>();
    for (const r of records) {
      for (const nr of r.nodeResults) {
        ...
        entry.total++;
        if (!nr.success) entry.fail++;
        nodeFailures.set(nr.nodeType, entry);
      }
    }

    const mostExpensiveNode = [...nodeCosts.entries()].sort(
      (a, b) => b[1] - a[1],
    )[0]?.[0] ?? 'n/a';
  ...
  private analyzeNewRecord(record: ExecutionRecord): void {
  ...
  private detectCostPatterns(record: ExecutionRecord): void {
    ...
    if (sameRuns.length >= 3) {
      const avgCost =
        sameRuns.reduce((s, r) => s + r.totalCost, 0) / sameRuns.length;
      if (avgCost > 0.50) {
        this.addInsight({
  ...
  private detectReliabilityPatterns(record: ExecutionRecord): void {
    const failedNodes = record.nodeResults.filter((n) => !n.success);
    for (const fn of failedNodes) {
      const allRuns = this.records.flatMap((r) =>
        r.nodeResults.filter((n) => n.nodeType === fn.nodeType),
      );
      const failRate =
        allRuns.filter((n) => !n.success).length / allRuns.length;
  ...
  private detectPerformancePatterns(record: ExecutionRecord): void {
    for (const nr of record.nodeResults) {
  ...
  private detectOrderingPatterns(): void {
    ...
    if (failedPipelines.length > 0 && successfulPipelines.length > 0) {
      ...
      const lintFirstSuccess = successfulPipelines.filter((r) => {
        const lintIdx = r.nodeResults.findIndex((n) =>
          n.nodeType.includes('Linter'),
        );
        const reviewIdx = r.nodeResults.findIndex((n) =>
          n.nodeType.includes('CodeReviewer'),
        );
