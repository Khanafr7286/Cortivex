...
const BUILT_IN_TEMPLATES: PipelineDefinition[] = [
  {
    name: 'code-review',
    version: '1.0.0',
...
export class PipelineStore {
  ...
  private get pipelinesPath(): string {
   ...
   */
  async save(pipeline: PipelineDefinition): Promise<string> {
   ...
   */
  async listSaved(): Promise<Array<{ name: string; description: string; path: string }>> {
    await this.ensureDir();
   ...
   */
  listTemplates(): Array<{ name: string; description: string; tags: string[] }> {
  ...
  async listAll(): Promise<
    Array<{ name: string; description: string; source: 'saved' | 'template' }>
  > {
