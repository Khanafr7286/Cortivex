...
import type { PipelineDefinition, PipelineTemplate, NodeDefinition } from '../types.js';
...
function sanitizePipelineName(name: string): string {
  ...
  if (!PIPELINE_NAME_PATTERN.test(name)) {
    throw new Error(
      'Pipeline name may only contain alphanumeric characters, hyphens, and underscores'
    );
  }
  return name;
}
...
export class PipelineLoader {
  ...
  async load(name: string): Promise<PipelineDefinition> {
  ...
  loadFromString(yamlContent: string): PipelineDefinition {
    ...
    const parsed = parseYaml(yamlContent, { maxAliasCount: 100 }) as unknown;
    ...
    for (const rawNode of rawNodes) {
      ...
      if (node['config'] && typeof node['config'] === 'object') {
        nodeDef.config = node['config'] as Record<string, unknown>;
      }
  ...
  validate(pipeline: PipelineDefinition): ValidationResult {
    ...
    for (const node of pipeline.nodes) {
      if (!nodeRegistry.has(node.type)) {
        errors.push(
          `Node "${node.id}" references unknown type "${node.type}". Available types: ${nodeRegistry.listIds().join(', ')}`
        );
      }
    }
    ...
    for (const node of pipeline.nodes) {
      if (node.depends_on) {
        for (const dep of node.depends_on) {
          if (!nodeIds.has(dep)) {
            errors.push(
              `Node "${node.id}" depends on "${dep}", which does not exist in the pipeline.`
            );
          }
        }
      }
    ...
    // Warnings
    const nodesWithoutDeps = pipeline.nodes.filter(
      (n) => !n.depends_on || n.depends_on.length === 0
    );
    if (nodesWithoutDeps.length > 3) {
      warnings.push(
        `${nodesWithoutDeps.length} nodes have no dependencies and will run in parallel. Ensure this is intentional.`
      );
    }
  ...
  private detectCycle(nodes: NodeDefinition[]): string | null {
