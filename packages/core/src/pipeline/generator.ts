/**
 * Pipeline generator — maps natural language descriptions to pipeline
 * definitions using keyword matching and common development patterns.
 */
...
interface KeywordMapping {
  keywords: string[];
  nodeType: string; // kebab-case id matching the registry
  priority: number;
}
...
interface PipelinePattern {
  ...
  name: string;
  description: string;
  nodes: Array<{ type: string; depends_on?: string[] }>;
}
 ...
 */
function buildPipelineFromPattern(
  name: string,
  description: string,
  ...
  const nodes: NodeDefinition[] = pattern.nodes.map((n) => {
  ...
  const maxDuration = Math.max(
    ...nodes.map((n) => nodeRegistry.get(n.type)?.avgDuration ?? 60),
  );
 ...
 */
function buildPipelineFromTypes(
  name: string,
  description: string,
  types: string[],
): PipelineDefinition {
  ...
  const nodes: NodeDefinition[] = types.map((type) => {
...
function extractTags(nodes: NodeDefinition[]): string[] {
