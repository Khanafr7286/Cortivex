...
export class InsightApplier {
     ...
     */
    apply(pipeline, insights) {
        // Deep clone the pipeline to avoid mutation
        let modified = structuredClone(pipeline);
        const changes = [];
    ...
    applyReorder(pipeline, insight) {
        const modified = structuredClone(pipeline);
        const suggestedOrder = insight.details['suggestedOrder'];
        ...
        if (nodeIndices[0].index > nodeIndices[1].index) {
            ...
            if (firstNode.depends_on) {
                firstNode.depends_on = firstNode.depends_on.filter((d) => d !== secondNode.id);
            }
            return {
                pipeline: modified,
                change: {
    ...
    applyModelSubstitution(pipeline, insight) {
        const modified = structuredClone(pipeline);
        const nodeType = insight.details['nodeType'];
        const suggestedModel = insight.details['suggestedModel'];
    ...
    applySkipNode(pipeline, insight) {
        const modified = structuredClone(pipeline);
        const nodeType = insight.details['nodeType'];
        if (!nodeType) {
            return {
                ...
                for (const dep of node.depends_on) {
                    if (skippedIds.has(dep)) {
                        ...
                        if (skippedNode?.depends_on) {
                            newDeps.push(...skippedNode.depends_on.filter((d) => !skippedIds.has(d)));
                        }
                    }
                    else {
                        newDeps.push(dep);
    ...
    applyAddNode(pipeline, insight) {
        const modified = structuredClone(pipeline);
        const suggestedNodeType = insight.details['suggestedNode'];
        ...
        const alreadyExists = modified.nodes.some((n) => n.type === suggestedNodeType);
        if (alreadyExists) {
            return {
                pipeline: modified,
                change: {
        ...
        if (suggestedNodeType.includes('security')) {
            ...
            const rootNodes = modified.nodes.filter((n) => !n.depends_on || n.depends_on.length === 0);
