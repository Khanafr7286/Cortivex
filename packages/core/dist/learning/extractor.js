...
export class PatternExtractor {
    ...
    findOrderPatterns(history) {
        ...
        for (const [pipeline, runs] of pipelineRuns.entries()) {
            if (runs.length < MIN_RUNS_FOR_INSIGHT)
                continue;
            ...
            const securityFirstRuns = runs.filter((r) => {
                ...
                const securityIdx = nodeTypes.findIndex((t) => t.includes('security'));
            ...
            if (securityFirstRuns.length >= 2 &&
                reviewFirstRuns.length >= 2) {
                const securityFirstSuccess = securityFirstRuns.filter((r) => r.success).length /
                    securityFirstRuns.length;
                const reviewFirstSuccess = reviewFirstRuns.filter((r) => r.success).length /
                ...
                if (securityFirstSuccess > reviewFirstSuccess + 0.1) {
                    insights.push({
                        ...
                        confidence: Math.min(securityFirstSuccess, securityFirstRuns.length / (securityFirstRuns.length + 2)),
        ...
        for (const [nodeType, modelMap] of nodeTypeModels.entries()) {
            ...
            for (let i = 0; i < models.length; i++) {
                for (let j = i + 1; j < models.length; j++) {
                    ...
                    if (statsA.total < 2 || statsB.total < 2)
                        continue;
                    ...
                    const cheaper = statsA.avgCost < statsB.avgCost ? modelA : modelB;
                    const expensive = statsA.avgCost < statsB.avgCost ? modelB : modelA;
                    const cheaperStats = statsA.avgCost < statsB.avgCost ? statsA : statsB;
                    const expensiveStats = statsA.avgCost < statsB.avgCost ? statsB : statsA;
                    const cheaperRate = statsA.avgCost < statsB.avgCost ? rateA : rateB;
                    const expensiveRate = statsA.avgCost < statsB.avgCost ? rateB : rateA;
                    if (cheaperRate >= expensiveRate - 0.05 &&
                        cheaperStats.avgCost < expensiveStats.avgCost * 0.7) {
                        const savings = ((expensiveStats.avgCost - cheaperStats.avgCost) /
                        ...
                        insights.push({
                            ...
                            confidence: Math.min(cheaperRate, (cheaperStats.total + expensiveStats.total) /
     ...
     */
    findSkipPatterns(groups) {
        const insights = [];
        for (const group of groups) {
            if (group.results.length < MIN_RUNS_FOR_INSIGHT)
                continue;
            // Check if a node always produces empty/trivial output for certain contexts
            const emptyOutputs = group.results.filter((r) => r.success && r.cost < 0.01);
            if (emptyOutputs.length / group.results.length >
                ...
                insights.push({
                    ...
                    confidence: emptyOutputs.length / group.results.length,
                    ...
                    details: {
                        ...
                        trivialRunPercentage: (emptyOutputs.length / group.results.length) * 100,
                    },
                    discoveredAt: new Date().toISOString(),
                });
            }
     ...
     */
    findReliabilityPatterns(groups) {
        const insights = [];
        for (const group of groups) {
            if (group.results.length < MIN_RUNS_FOR_INSIGHT)
                continue;
            const failureRate = group.results.filter((r) => !r.success).length /
            ...
            if (failureRate > 0.3) {
                ...
                const topError = [...errorGroups.entries()].sort((a, b) => b[1] - a[1])[0];
                insights.push({
                    id: uuidv4(),
                    ...
                    confidence: Math.min(failureRate, group.results.length / (group.results.length + 2)),
                    basedOnRuns: group.results.length,
                    action: 'skip_node',
                    details: {
                        nodeType: group.nodeType,
     ...
     */
    findCostOptimizations(groups) {
        const insights = [];
        for (const group of groups) {
            if (group.results.length < MIN_RUNS_FOR_INSIGHT)
                continue;
            const avgCost = group.results.reduce((sum, r) => sum + r.cost, 0) /
                group.results.length;
            const avgDuration = group.results.reduce((sum, r) => sum + r.duration, 0) /
                group.results.length;
            ...
            for (const exec of group.executions) {
                ...
                if (totalCost > 0 && avgCost / totalCost > 0.5) {
                    insights.push({
                        ...
                        confidence: Math.min(0.9, group.results.length / (group.results.length + 2)),
                        basedOnRuns: group.results.length,
                        action: 'substitute_model',
                        details: {
                            nodeType: group.nodeType,
     ...
     */
    findContextualPatterns(history) {
        const insights = [];
        ...
        if (authRuns.length >= MIN_RUNS_FOR_INSIGHT) {
            const authSuccessRate = authRuns.filter((r) => r.success).length / authRuns.length;
            // Check if security scanner runs improve success rate for auth repos
            const authWithSecurity = authRuns.filter((r) => r.nodeResults.some((n) => n.nodeType.includes('security')));
            const authWithoutSecurity = authRuns.filter((r) => !r.nodeResults.some((n) => n.nodeType.includes('security')));
            if (authWithSecurity.length >= 2 &&
                authWithoutSecurity.length >= 2) {
                const withSecRate = authWithSecurity.filter((r) => r.success).length /
                    authWithSecurity.length;
                const withoutSecRate = authWithoutSecurity.filter((r) => r.success).length /
                ...
                if (withSecRate > withoutSecRate + 0.15) {
                    insights.push({
                        ...
                        confidence: Math.min(withSecRate, (authWithSecurity.length + authWithoutSecurity.length) /
                            (authWithSecurity.length +
                                authWithoutSecurity.length +
                                4)),
                        basedOnRuns: authWithSecurity.length + authWithoutSecurity.length,
                        action: 'add_node',
                        details: {
        ...
        for (const [framework, runs] of frameworkRuns.entries()) {
            ...
            const overallRate = history.filter((r) => r.success).length / history.length;
            if (successRate < overallRate - 0.15) {
                insights.push({
                    ...
                    confidence: Math.min(0.8, runs.length / (runs.length + 3)),
