...
describe('MeshManager', () => {
    ...
    describe('claim()', () => {
        it('writes a claim file to the mesh directory', async () => {
            const claim = await manager.claim('agent-1', 'node-a', 'run-001', ['file1.ts', 'file2.ts']);
    ...
    describe('sanitizeAgentId()', () => {
        it('rejects path traversal attempts', async () => {
            await expect(manager.claim('../../../etc/passwd', 'node-a', 'run-001', ['f.ts'])).rejects.toThrow(/Agent ID may only contain/);
        });
        it('rejects empty agent IDs', async () => {
            await expect(manager.claim('', 'node-a', 'run-001', ['f.ts'])).rejects.toThrow(/Agent ID must be a non-empty string/);
        });
        it('rejects agent IDs with slashes', async () => {
            await expect(manager.claim('agent/malicious', 'node-a', 'run-001', ['f.ts'])).rejects.toThrow(/Agent ID may only contain/);
        });
        it('rejects excessively long agent IDs', async () => {
            const longId = 'a'.repeat(129);
            await expect(manager.claim(longId, 'node-a', 'run-001', ['f.ts'])).rejects.toThrow(/Agent ID must not exceed 128 characters/);
        });
        it('accepts valid agent IDs with dots, hyphens, and underscores', async () => {
            const claim = await manager.claim('agent-1_v2.0', 'node-a', 'run-001', ['f.ts']);
            expect(claim.agentId).toBe('agent-1_v2.0');
        });
    });
});
//# sourceMappingURL=mesh-manager.test.js.map